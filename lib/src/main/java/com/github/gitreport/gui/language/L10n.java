...
public class L10n {
	 ...
	 *
	 * If user didn't set language preference the resources are loaded according
	 * to default locale. Otherwise according to the user preference.
	 *
	 * @throws IOException
	...
	public static boolean load(String locale) throws IOException {
		...
		if (locale.equals("DUMMY_DEFAULT")) {
			...
			default: // this should never happen
				loc = Locale.getDefault();
				logger.error(String.format(Constant.LOAD_LANGUAGE_PROBLEM,
						locale));
			}
		}
		try {
			if (locale.equals("file")) {
				resource = new PropertyResourceBundle(new InputStreamReader(
						new FileInputStream(Config.MANAGER_FOLDER
								+ File.separator + "gitreport.properties"),
						"UTF-8"));
			} else {
		...
		try {
			defaultResource = Utf8ResourceBundle.getBundle(RESOURCE_NAME,
	 ...
	 *
	 * Removes the first ampersand sign (&) because it is assumed that it is an
	 * indiaction of a mnemonic.
	 *
	 * @param key
	 *            Key of the required value
	 * @throws NullPointerException
	 *             in case that <code>load()</code> wasn't called first or it
	 *             failed.
	 * @throws MissingResourceException
	 *             in case the <code>key</code> is not defined!
	 */
		...
		try {
			...
			try {
				logger.warn("The key \"" + key
						+ "\" is not defined in the property file for "
						+ currentLocale);
				StringBuffer sb = new StringBuffer(
						defaultResource.getString(key));
				int i = sb.indexOf("&");
				if (i >= 0) {
					sb.deleteCharAt(i);
				}
				...
				logger.warn("The key \"" + key
						+ "\" is not defined in the DEFAULT property file!");
	 ...
	 * @param key
	 *            Key of the required value
	 * @trhows NullPointerException in case that load() wasn't called first or
	 *         it failed.
	 */
